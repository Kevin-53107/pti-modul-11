...
export class Article {
  ...
  //article nantinya akan memilih vote up dan vote down
  ...
  domain(): string{
      ...
      // https://google.com/angular7, setelah dilakukan split hasilnya
      // akan menjadi www.google.com mulai dari slash akan dipotong(split)

      try{
          const domainAndPath: string = this.link.split(separator '//')[1];
          return domainAndPath.split(separator '/')[0];
      }
      catch (err){
          return null;
      }
